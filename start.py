...
def uart_get():
    ...
    while ser.in_waiting:
        ret_str += ser.read(1).decode('utf-8')
    return ret_str
...
if __name__ == "__main__":
    ...
    try:
        while True:
            if debug_enable:
                print("\n[Debug mode]")
                mode = int(input("Enter mode: 1)Tune 2)Play 3)Tune Result 4)Exit Debug: "))
            else:
                mode = int(input("Enter mode: 1)Tune 2)Play 3)Tune Result 4)Debug: "))
